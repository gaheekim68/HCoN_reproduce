...
class AddHypergraphSelfLoops:
    ...
    def __call__(self, data):
        ...
        num_nodes = int(num_nodes[0])  # 리스트 [2708]이면 -> 2708로 바꿈
        ...
        num_hyperedges = int(num_hyperedges[0])  # 리스트로 들어왔으면
...
class HypergraphDataset(InMemoryDataset):
    ...
    def __init__(self, root, name,  path_to_download='./raw_data',
        ...
        num_nodes = int(self.data.num_nodes[0])
        num_hyperedges = int(self.data.num_hyperedges[0])
        assert (num_nodes + num_hyperedges - 1) == self.data.edge_index.max().item()
